...
# driver = webdriver.Firefox()python.formatting.provider
...
def LoginWeibo(username, password):
    try:
        ...
        elem_sub = driver.find_element_by_xpath(
            "//input[@class='W_btn_a btn_34px']")
...
def handlePage():
    while True:
        ...
        if checkContent():
            ...
            if checkNext():
                # 拿到下一页按钮
                next_page_btn = driver.find_element_by_css_selector(
                    "#pl_feedlist_index > div.m-page > div > a.next")
                next_page_btn.click()
            else:
                print("no Next")
                break
...
def checkqw():
    try:
        driver.find_element_by_xpath(
            ".//div[@class='content']/p[@class='txt']/a")
        flag = True
    except:
        flag = False
    return flag
...
def getContent():
    ...
    for i in range(len(nodes)):
        ...
        try:
            ...
            BZNC = nodes[i].find_element_by_xpath(
                ".//div[@class='content']/p[@class='txt']").get_attribute("nick-name")
        ...
        try:
            BZZY = nodes[i].find_element_by_xpath(
                ".//div[@class='content']/div[@class='info']/div[2]/a").get_attribute("href")
        ...
        try:
            ...
            WBRZ = nodes[i].find_element_by_xpath(
                ".//div[@class='info']/div/a[contains(@title,'微博')]").get_attribute('title')  # 若没有认证则不存在节点
        except:
            WBRZ = ''
        print('微博认证:', WBRZ)
        dic[i].append(WBRZ)
        ...
        # 判断展开全文和网页链接是否存在
        try:
            nodes[i].find_element_by_xpath(
                ".//div[@class='content']/p[@class='txt']/a[@action-type='fl_unfold']").is_displayed()
            flag = True
        except:
            flag = False
        # 获取微博内容
        try:
            if flag:
                nodes[i].find_element_by_xpath(
                    ".//div[@class='content']/p[@class='txt']/a[@action-type='fl_unfold']").click()
                time.sleep(1)
                WBNR = nodes[i].find_element_by_xpath(
                    ".//div[@class='content']/p[2]").text.replace("\n", "")
                # 判断发布位置是否存在
                try:
                    nodes[i].find_element_by_xpath(
                        ".//div[@class='content']/p[@class='txt']/a/i[@class='wbicon']").is_displayed()
                    flag = True
                except:
                    flag = False
                # 获取微博发布位置
                try:
                    if flag:
                        pattern = nodes[i].find_elements_by_xpath(
                            ".//div[@class='content']/p[2]/a[i[@class='wbicon']]")
                ...
                WBNR = nodes[i].find_element_by_xpath(
                    ".//div[@class='content']/p[@class='txt']").text.replace("\n", "")
                # 判断发布位置是否存在
                try:
                    nodes[i].find_element_by_xpath(
                        ".//div[@class='content']/p[@class='txt']/a/i[@class='wbicon']").is_displayed()
                ...
                try:
                    if flag:
                        pattern = nodes[i].find_elements_by_xpath(
                            ".//div[@class='content']/p[@class='txt']/a[i[@class='wbicon']]")
                        if isinstance(pattern, list):
                            text = [p.text for p in pattern]
                            FBWZ = [loc for loc in [re.findall(
                                '^2(.*$)', t) for t in text] if len(loc) > 0][0][0]
        ...
        try:
            ...
            FBSJ = nodes[i].find_element_by_xpath(
                ".//div[@class='content']/p[@class='from']/a[1]").text
        except:
            FBSJ = ''
        print('发布时间:', FBSJ)
        dic[i].append(FBSJ)
        ...
        try:
            ...
            WBDZ = nodes[i].find_element_by_xpath(
                ".//div[@class='content']/p[@class='from']/a[1]").get_attribute("href")
        except:
            WBDZ = ''
        print('微博地址:', WBDZ)
        dic[i].append(WBDZ)
        ...
        try:
            WBLY = nodes[i].find_element_by_xpath(
                ".//div[@class='content']/p[@class='from']/a[2]").text
        except:
            WBLY = ''
        print('微博来源:', WBLY)
        dic[i].append(WBLY)
        ...
        try:
            ZF_TEXT = nodes[i].find_element_by_xpath(
        ...
        try:
            ...
            PL_TEXT = nodes[i].find_element_by_xpath(
                ".//a[@action-type='feed_list_comment']").text  # 可能没有em元素
        ...
        try:
            ZAN_TEXT = nodes[i].find_element_by_xpath(
                ".//a[@action-type='feed_list_like']//em").text  # 可为空
